...
class ApiClient(object):
    ...
    def get_geoid_data(self, geoid):
        data = self._get('/1.0/geo/tiger2014/{}'.format(geoid))
        return data
    ...
    def get_data(self, table_ids, geo_ids, acs='latest'):
        ...
        path = '/1.0/data/show/{}'.format(acs)
        map=dict(table_ids=table_ids, geo_ids=geo_ids)
        result = self._get(path, params=map)
        return result
...
def iframe_profile(geoid, acs='latest'):
    api = ApiClient(settings.API_URL)
